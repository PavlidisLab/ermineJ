...
public abstract class AbstractTreeModel implements TreeModel {
    ...
    protected void fireNewRoot() {
        ...
        /*
         * Undocumented. I think it is the only reasonable/possible solution to use use null as path if there is no
         * root. TreeModels without root aren't important anyway, since JTree doesn't support them (yet).
         */
    ...
    /**
     * Call when everything but the root has changed. Only may be called when the root is not null. Otherwise there
     * isn't a structure to have changed.
     */
    ...
    protected void fireChildrenAdded( TreePath parentPath, int[] indices, Object[] children ) {
        ...
        for ( int i = pairs.length - 2; i >= 0; i -= 2 ) {
            if ( pairs[i] == TreeModelListener.class ) {
                if ( e == null ) e = new TreeModelEvent( this, parentPath, indices, children );
