...
public class SpecFunc {
  ...
  public static double gammaln(double xx) {
    double x, y, tmp, ser;
    double[] cof = {
        76.18009172947146, -86.50532032941677, 24.0140982408391,
  ...
  public static double betacf(double a, double b, double x) {
    ...
    if (m > MAXIT) {
      System.err.println("a (" + a + ") or b (" + b + ") too big, or MAXIT (" +
                         MAXIT + ") too small in betacf");
  ...
  public static double binomial_prob(int successes, int trials, double p) {
    return binomial_coeff(trials, successes) * Math.pow(p, successes) *
  ...
  public static double binomialCumProb(int successes, int trials, double p) {
    ...
    if (pval == 0.0 || Double.isNaN(pval)) {
      System.err.println("Binomial returned probability of zero, or NAN (" +
                         pval + ") for " + successes + " successes, " + trials +
                         " trials, p=" + p);
      return LIMIT;
    }
    else if (Double.isInfinite(pval)) {
      System.err.println("Binomial returned probability of infinity (" + pval +
                         ") for " + successes + " successes, " + trials +
                         " trials, p=" + p + ", diagnostic follows:");
      pval = LIMIT;
      for (i = successes + 1; i <= trials; i++) {
        pval += binomial_prob(i, trials, p);
        System.err.println(binomial_prob(i, trials, p) + " " + i + " " + trials +
                           " " + p + ", Current p=" + pval);
      }
      System.err.println("----");
      return pval;
  ...
  static private double polevl(double x, double coef[], int N) throws
      ArithmeticException {
  ...
  static private double p1evl(double x, double coef[], int N) throws
      ArithmeticException {
