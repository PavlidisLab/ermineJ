...

{
   ...
   */
  public void chooserandom(double[] randomvals, double[] sourcedata, int[] deck,
                           int max, int n) {
    int rand;
    int i;
    int temp;
   ...
   */
  public void matrix_row_stdevs(Matrix M, double[] means, double[] ssqs,
                                double[] stdevs) {
    int i;
    for (i = 0; i < M.get_num_rows(); i++) {
      ...
      for (int j = l; j < M.get_num_rows(); j++) {
        if (j == i) {
          ...
          for (int k = 0; k < M.get_num_cols(); k++) {
            sxy += (M.get_matrix_val(i, k) - means[i]) *
                (M.get_matrix_val(j, k) - means[j]);
          }
   ...
   */
  public static double calculate_quantile(int index, double[] random_class,
                                          int size)
   ...
   */
  public static double calculate_mean_above_quantile(int quantile,
  ...
  public static double hypergeometric(int positives, int successes,
    ...
    return SpecFunc.binomial_coeff(positives, successes) *
        SpecFunc.binomial_coeff(negatives, failures) /
        SpecFunc.binomial_coeff(positives + negatives, successes + failures);
  }
  ...
  public static LinkedHashMap rankOf(Map m) {
     ...
     * pvalue and the gene id */while (itr.hasNext()) {
  ...
  public static void main(String[] args) {
    ...
    double binom_pval = SpecFunc.binomialCumProb(b, d,
                                                 (double) a / (double) (a + c));
    ...
    System.err.println("one-tailed normal for " + 1.0 + ": " +
                       SpecFunc.normdist(1.0));
    System.err.println("one-tailed normal for " + 2.0 + ": " +
                       SpecFunc.normdist(2.0));
    System.err.println("one-tailed normal for " + 3.0 + ": " +
                       SpecFunc.normdist(3.0));
    System.err.println("one-tailed normal for " + 4.0 + ": " +
                       SpecFunc.normdist(4.0));
    System.err.println("one-tailed normal for " + 5.0 + ": " +
                       SpecFunc.normdist(5.0));
  }
