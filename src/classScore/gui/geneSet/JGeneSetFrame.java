...
public class JGeneSetFrame extends JFrame {
	...
	private void jbInit() throws Exception {
		...
		m_table.addMouseMotionListener(new JGeneSetFrame_m_mouseMotionListener(
				this));
	...
	void m_saveImageMenuItem_actionPerformed(ActionEvent e) {
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			try {
				...
				System.err.println("IOException error saving png to "
						+ filename);
			}
		}
	...
	void m_saveDataMenuItem_actionPerformed(ActionEvent e) {
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			try {
				...
				System.err.println("IOException error saving data to "
						+ filename);
			}
		}
		// else canceled by user
	...
	void m_colorRangeSlider_stateChanged(ChangeEvent e) {
		...
		if (normalized) {
			...
			double rangeMax = m_matrixDisplay.getMax()
					- m_matrixDisplay.getMin();
			double zoomFactor = COLOR_RANGE_SLIDER_RESOLUTION / rangeMax;
			double range = value / zoomFactor;
	...
	void table_mouseReleased(MouseEvent e) {
		if (SwingUtilities.isLeftMouseButton(e)) {
			...
			if (m_table.getValueAt(i, j) != null
					&& j == m_table.getColumnCount() - 2) {
	...
	void table_mouseMoved(MouseEvent e) {
		...
		if (i > 0 && j > 0 && j == m_table.getColumnCount() - 2
				&& m_table.getValueAt(i, j) != null) {
...
class JGeneSetFrame_m_mouseMotionListener implements
