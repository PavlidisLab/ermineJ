...
public class NewClass {
         ...
         try {
            ...
            if (filetype == 1) {
               ...
               for (Iterator it = genes.iterator(); it.hasNext(); ) {
                  probeSet.addAll(outerframe.imaps.geneData.getGeneProbeList( (String) it.next()));
               }
   ...
   public void saveClass(String folder, int type)
   {
      try
      {
         ...
         BufferedWriter out = new BufferedWriter(new FileWriter(folder+"classes"+File.separatorChar+fileid+"-class.txt", false));
         out.write(filetype+"\n");
         out.write(id+"\n");
         out.write(filedesc+"\n");
