...
public class histogram {
   ...
   private double bin_size = 0.002; // todo: set this automatically?, so there
   ...
   private DenseDoubleMatrix2DNamed M = null; // holds the actual histograms. Each
                                         // row is a histogram.
   ...
   public histogram( int number_of_class, int min_class_size,
      ...
      M = new DenseDoubleMatrix2DNamed( number_of_class, number_of_bins + 1 );
   }
   ...
   public void set_number_of_runs( int runs ) {
      number_of_runs = runs;
      minPval = 0.5 / ( double ) number_of_runs; // the best possible pvalue for
                                                 // a class.
   ...
   public void update( int row, double value ) {
      ...
      int thebin = ( int ) Math.floor( ( value - hist_min )
            / ( double ) bin_size );
      ...
      if ( thebin > number_of_bins - 1 ) { // this shouldn't happen since we
   ...
   public void tocdf() {
      ...
      DoubleArrayList sums = MatrixRowStats.sums(M);
      
      for ( int i = 0; i < M.rows(); i++ ) { // for each histogram (class size)

         double total = sums.get(i);
         double sum = 0.0;
         for ( int j = M.columns() - 1; j >= 0; j-- ) { // for each bin in this
                                                        // histogram. Go from
                                                        // right to avoid
                                                        // roundoff problems
                                                        // where it matters.
            sum += M.getQuick( i, j ) / total;
            M.setQuick( i, j, sum );
         }
      }
   ...
   public String toString() {
      return "There are " + number_of_bins
            ...
            + hist_max + ", the minimum is " + hist_min + "."
            + " Min class is " + min_class_size + ".";
   }
   ...
   public double get_val( int class_size, double rawscore ) {
      if ( rawscore > hist_max || rawscore < hist_min ) { // sanity check.
         System.err
               .println( "Warning, a rawscore yielded a bin number which was out of the range: "
                     + rawscore );
         return -1.0;
      } else {
