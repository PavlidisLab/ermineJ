...
/*****************************************************************************************/
// input format will be pval_file,method,class_max_size,class_min_size,number_of_runs,quantile,range
...
/*****************************************************************************************/
    {
	Pval_parse parser = new Pval_parse(filename_pval);
	chip_pval= new LinkedHashMap();
	chip_pval=parser.get_map();
	ug_pval_map = new HashMap();
	weight_on =(Boolean.valueOf(wt_check)).booleanValue();
	pvals = parser.get_pval();
	method = in_method;
    }
...
/*****************************************************************************************/
	...
	int i,j,k;
	int gene_length = weight_on ? Array.getLength(ug_pval_arr) : Array.getLength(pvals);
	int number_of_class = class_max_size - class_min_size + 1;
	int count =0;
	int[] random = null;
	double[] random_class = new double[gene_length];
	double[] in_pval = weight_on? ug_pval_arr : pvals;
	...
	for (i = class_min_size; i<class_max_size;i++)
		...
		for (k =0 ; k< number_of_runs;k++)
			...
			//statistics.chooserandom_2(random, gene_length-1, i);
			...
			//System.out.flush();
			mat.set_matrix_val(count,k,total);
		    }
		count++;
	
	    }
	//convert matrix of values into a histogram
...
/*****************************************************************************************/
	    {
                ...
                while(ugMapItr.hasNext()){
                    ...
                    int in_size = 0;
                    while(pbItr.hasNext()){
                        Object key = chip_pval.get(pbItr.next());
                        if(key != null){	
